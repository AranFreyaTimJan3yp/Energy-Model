...
#        print(max_sell_price-max_buy_price)
        for t in range(48):
            ...
            balancing.append(0)
            for storageid in storage:                   #run through all storage facilities               
                if t in max_sell_price_t_list[0:7]:     #Decide how many time periods will be used for forced export
...
def cvxopt(storage_size_list,house_pv,sp_pv,pandr_pv):
    ...
    for storage_max in storage_size_list:
        ...
        pvl=house_pv*0.9*solar_data()+sp_pv*0.9*solar_data_flat()+pandr_pv*0.9*solar_data_10()
...
def conditional(storage_size_list,house_pv,sp_pv,pandr_pv):
    ...
    for storage_max in storage_size_list:       
        ...
        load={}
        storage={}
        pv=create_pv(pv,house_pv*0.9,house_pv*56.95,solar_data())
...
house_pv=231600*0.2*0.9
sp_pv=88000*0.2*0.9
pandr_pv=11520*0.2*0.9
#storage_size_list=np.array(list(range(10,12)))*2000
...
#plt.ylim([0,7])
ax.set_title('Annual Cost of Conditional Statement System')
#fig.legend(loc='upper center', bbox_to_anchor=(0.7, 0.4), ncol=1)
#fig.legend()
plt.grid()
